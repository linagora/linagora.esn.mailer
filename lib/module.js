...
module.exports = function(dependencies, options) {
   ...
   */
  var attachments = function(template, filter) {
        ...
        template(templateName, locals, function(err, html, text) {
          ...
          mailconfig(function(err, data) {
            ...
            if (hasAttachments(templateName)) {
              var filter = locals.filter;
              if (Array.isArray(message.attachments)) {
                message.attachments = message.attachments.concat(attachments(templateName, filter));
              } else {
                message.attachments = attachments(templateName, filter);
              }
            }
  ...
  lib.send = function(message, done) {
    ...
    if (!message.text && !message.html) {
      return done(new Error('Message content can not be null'));
    }
